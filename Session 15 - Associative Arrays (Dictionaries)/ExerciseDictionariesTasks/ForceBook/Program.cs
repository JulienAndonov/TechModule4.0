        ...
        static void Main(string[] args)
        {
            ...
            while (true)
            {
                ...
                if (input.Contains("->"))
                {
                    ...
                    string oldSideNew = forceUsers.Values.Where(x => x.Contains(name)).ToString();
                    string newSide = entries[1];

                    if (!forceUsers.ContainsKey(newSide))
                    {
                        forceUsers[newSide] = new List<string>();
                    }

                    if (!forceUsers[newSide].Contains(name))
                    {
                        forceUsers[newSide].Add(name);
